...
/// copy-and-paste from https://github.com/983/Num/blob/master/num.hpp
...
inline void add_divisors(map<int,int> &numbers, int number) {
    while(number != 1) {
        numbers[divisor[number]]++;
...
int main() {
    ...
    //denominator
    for(int i = 1 ; i <= n; ++i) add_divisors(b, i);

    //the upper part
    //first - k over n const points.
    for(int i = 1; i <= n; ++i) add_divisors(a, i);
    for(int i = 1; i <= (n-k); ++i) add_divisors(b, i);
    for(int i = 1; i <= k; ++i) add_divisors(b, i);
    //now arrangement of non-const points
    for(int i = 1; i <= (n-k-1); ++i) add_divisors(a, i);

    //gcd :)
    for(auto d : b) {
        int mini = min(a[d.first], d.second);
        a[d.first] -= mini;
        b[d.first] -= mini;
    }
    ...
    Num resa = 1, resb = 1;
    for(auto d : a) {
        for(int i = 0 ; i < d.second; ++i) resa *= d.first;
    }
    for(auto d : b) {
        for(int i = 0 ; i < d.second; ++i) resb *= d.first;
    }

    vector<char> resa_string, resb_string;
    resa.print(resa_string);
    resb.print(resb_string);

    printf("%s/%s", &resa_string[0], &resb_string[0]);
